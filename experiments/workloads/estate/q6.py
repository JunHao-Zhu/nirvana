...
if __name__ == "__main__":
    ...
    logger = create_logger(log_dir="log/estate", log_name=f"q6_r{ROUND}{ablation_suffix}")
    ...
    logger.info(f"Q6: Compute the average price for the estates that seem to have a yard.")
    ...
    df.semantic_filter(user_instruction="Observed from the house picture, whether the house has a yard or not.", input_columns=["image"])
    ...
    if ROUND == 1:
        output.to_csv(f"workloads/estate_output/q6_out{ablation_suffix}.csv", index=False)
        logger.info(f"saved output to workloads/estate_output/q6_out{ablation_suffix}.csv...")
