...
if __name__ == "__main__":
    ...
    logger = create_logger(log_dir="log/estate", log_name=f"q8_r{ROUND}{ablation_suffix}")
    ...
    df.semantic_map(user_instruction="Extract amenities from the detail about the estate.", input_columns=["Details"], output_columns=["Amenities"])
    ...
    if ROUND == 1:
        output.to_csv(f"workloads/estate_output/q8_out{ablation_suffix}.csv", index=False)
        logger.info(f"saved output to workloads/estate_output/q8_out{ablation_suffix}.csv...")
