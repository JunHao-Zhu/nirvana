...
if __name__ == "__main__":
    ...
    logger = create_logger(log_dir="log/estate", log_name=f"q10_r{ROUND}{ablation_suffix}")
    ...
    logger.info(f"Q10: Compute the lowest price for the detached duplex that has more than 3 bedrooms and less than 6 bedrooms.")
    ...
    df.semantic_filter(user_instruction="Whether the estate has more than 3 bedrooms.", input_columns=["Title"])
    df.semantic_filter(user_instruction="Whether the estate has less than 6 bedrooms.", input_columns=["Title"])
    df.semantic_filter(user_instruction="Whether the estate is a detached duplex.", input_columns=["Title"])
    ...
    if ROUND == 1:
        output.to_csv(f"workloads/estate_output/q10_out{ablation_suffix}.csv", index=False)
        logger.info(f"saved output to workloads/estate_output/q10_out{ablation_suffix}.csv...")
