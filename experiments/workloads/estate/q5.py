...
if __name__ == "__main__":
    ...
    logger = create_logger(log_dir="log/estate", log_name=f"q5_r{ROUND}{ablation_suffix}")
    ...
    logger.info(f"Q5: Extract Amenities of the estate that has more than 3 bedrooms but less than 6 bedrooms.")
    df.semantic_filter(user_instruction="Whether the estate has more than 3 bedrooms", input_columns=["Title"])
    df.semantic_map(user_instruction="Extract Amenities of the estate from the estate details.", input_columns=["Details"], output_columns=["Amenities"])
    ...
    if ROUND == 1:
        output.to_csv(f"workloads/estate_output/q5_out{ablation_suffix}.csv", index=False)
        logger.info(f"saved output to workloads/estate_output/q5_out{ablation_suffix}.csv...")
