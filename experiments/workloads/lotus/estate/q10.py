...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/estate_lotus", log_name=f"q10_r{ROUND}")
    ...
    df.reset_index(drop=True, inplace=True)
    df = df.sem_filter("Based on {Title}, whether the estate is a detached duplex.", cascade_args=cascade_args)
    df.reset_index(drop=True, inplace=True)
    ...
    if ROUND == 1:
        df.to_csv("workloads/estate_output/q10_out_lotus.csv", index=False)
        logger.info(f"saved output to workloads/estate_output/q10_out_lotus.csv...")
