...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/estate_lotus", log_name=f"q12_r{ROUND}")
    ...
    df.semantic_filter(user_instruction="Is there a gym in the estate.", input_column="Amenities")
    df.semantic_filter(user_instruction="Is the estate located in Lekki, Lagos.", input_column="Location")
    ...
    df = df.sem_filter("Is there a gym in the {Amenities}.", cascade_args=cascade_args)
    df.reset_index(drop=True, inplace=True)
    df = df.sem_filter("Is the {Location} in Lekki, Lagos.", cascade_args=cascade_args)
    df.reset_index(drop=True, inplace=True)
    ...
    if ROUND == 1:
        df.to_csv("workloads/estate_output/q12_out_lotus.csv", index=False)
        logger.info(f"saved output to workloads/estate_output/q12_out_lotus.csv...")
