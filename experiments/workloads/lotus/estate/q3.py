...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/estate_lotus", log_name=f"q3_r{ROUND}")
    ...
    df = df.sem_filter("Whether the {Location} is located in Ajah, Lagos.", cascade_args=cascade_args)
    end_time = time.time()
    runtime = end_time - start_time
    cost = lm.stats.physical_usage.total_cost + proxy_lm.stats.physical_usage.total_cost

    if ROUND == 1:
        df.to_csv("workloads/estate_output/q3_out_lotus.csv", index=False)
        logger.info(f"saved output to workloads/estate_output/q3_out_lotus.csv...")
