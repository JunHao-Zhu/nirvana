...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/estate_lotus", log_name=f"q8_r{ROUND}")
    ...
    logger.info(f"Q8: Extract amenities of the estates that have 2 or 3 bedrooms from the estate details.")
    start_time = time.time()
    df = df.sem_map("Extract amenities from the {Details} about the estate.", suffix="Amenities")
    df = df.sem_filter("Based on {Title}, whether the estate has 2 or 3 bedrooms.", cascade_args=cascade_args)
    end_time = time.time()
    runtime = end_time - start_time
    cost = lm.stats.physical_usage.total_cost + proxy_lm.stats.physical_usage.total_cost

    if ROUND == 1:
        df.to_csv("workloads/estate_output/q8_out_lotus.csv", index=False)
        logger.info(f"saved output to workloads/estate_output/q8_out_lotus.csv...")
