...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/imdb_lotus", log_name=f"q9_r{ROUND}")
    ...
    df = df.sem_map("According to the movie plot in {Plot}, extract the genre(s) of each movie.", suffix="Genre")
    df = df.sem_filter("{IMDB_rating} is higher than 8.5?", cascade_args=cascade_args)
    df.reset_index(drop=True, inplace=True)
    df = df.sem_filter("{IMDB_rating} is lower than 9?", cascade_args=cascade_args)
    df.reset_index(drop=True, inplace=True)
    df = df.sem_filter("The {Genre} belongs to crime movies?", cascade_args=cascade_args)
    df.reset_index(drop=True, inplace=True)
    df = df.sem_agg("Find the maximum rating in the rest movies based on {IMDB_rating}.")
    end_time = time.time()
    runtime = end_time - start_time
    cost = lm.stats.physical_usage.total_cost + proxy_lm.stats.physical_usage.total_cost

    if ROUND == 1:
        df.to_csv("workloads/imdb_output/q9_out_lotus.csv", index=False)
        logger.info(f"saved output to workloads/imdb_output/q9_out_lotus.csv...")
