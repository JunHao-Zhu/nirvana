...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/imdb_lotus", log_name=f"q5_r{ROUND}")
    ...
    df = df.sem_agg("Compute the total box office gross based on {BoxOffice}.")
    end_time = time.time()
    runtime = end_time - start_time
    cost = lm.stats.physical_usage.total_cost + proxy_lm.stats.physical_usage.total_cost

    if ROUND == 1:
        df.to_csv("workloads/imdb_output/q5_out_lotus.csv", index=False)
        logger.info(f"saved output to workloads/imdb_output/q5_out_lotus.csv...")
