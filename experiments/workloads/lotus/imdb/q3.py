...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/imdb_lotus", log_name=f"q3_r{ROUND}")
    ...
    df = df.sem_filter("Whether the movie {Poster} image is in the dark style?", cascade_args=cascade_args)
    end_time = time.time()
    runtime = end_time - start_time
    cost = lm.stats.physical_usage.total_cost + proxy_lm.stats.physical_usage.total_cost

    if ROUND == 1:
        df.to_csv("workloads/imdb_output/q3_out_lotus.csv", index=False)
        logger.info(f"saved output to workloads/imdb_output/q3_out_lotus.csv...")
