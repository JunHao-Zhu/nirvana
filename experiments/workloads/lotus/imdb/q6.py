...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/imdb_lotus", log_name=f"q6_r{ROUND}")
    ...
    df = df.sem_filter("Whether Quentin Tarantino is in {Director}?", cascade_args=cascade_args)
    ...
    if ROUND == 1:
        df.to_csv("workloads/imdb_output/q6_out_lotus.csv", index=False)
        logger.info(f"saved output to workloads/imdb_output/q6_out_lotus.csv...")
