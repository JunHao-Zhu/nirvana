...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/steam_lotus", log_name=f"q6_r{ROUND}")
    ...
    try:
        df = df.sem_filter("Is MacOS in the list of supported {platforms}.", cascade_args=cascade_args)
        df.reset_index(drop=True, inplace=True)
        df = df.sem_agg("Find the lowest price in {discounted_price}.")
    ...
    if ROUND == 1:
        df.to_csv("workloads/steam_output/q6_out_lotus.csv", index=False)
        logger.info(f"saved output to workloads/steam_output/q6_out_lotus.csv...")
