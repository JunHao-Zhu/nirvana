...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/steam_lotus", log_name=f"q8_r{ROUND}")
    ...
    try:
        df = df.sem_filter("The {metacriticts} score is higher than 90", cascade_args=cascade_args)
        df.reset_index(drop=True, inplace=True)
        df = df.sem_filter("The {developer} has only one developer", cascade_args=cascade_args)
        df.reset_index(drop=True, inplace=True)
        df = df.sem_agg("Count the number of games based on {title}.")
    ...
    if ROUND == 1:
        df.to_csv("workloads/steam_output/q8_out_lotus.csv", index=False)
        logger.info(f"saved output to workloads/steam_output/q8_out_lotus.csv...")
