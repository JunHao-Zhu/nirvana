...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/steam_lotus", log_name=f"q12_r{ROUND}")
    ...
    try:
        ...
        df = df.sem_filter("The {graphic_style} of the game is cartoon.", cascade_args=cascade_args)
        df.reset_index(drop=True, inplace=True)
        df = df.sem_map("Extract the genre from the brief {description} of the game.", suffix="genre")
        df = df.sem_filter("The {genre} of the game is about adventure.", cascade_args=cascade_args)
        df.reset_index(drop=True, inplace=True)
        df = df.sem_agg("Find the earliest date from {release_date}.")
    ...
    if ROUND == 1:
        df.to_csv("workloads/steam_output/q12_out_lotus.csv", index=False)
        logger.info(f"saved output to workloads/steam_output/q12_out_lotus.csv...")
