...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/steam_lotus", log_name=f"q10_r{ROUND}")
    ...
    try:
        df = df.sem_map("Given the video game a binary review (positive or negative) based on the {overall_reviews}.", suffix="comments")
        df = df.sem_filter("The {comments} of the game is positive.", cascade_args=cascade_args)
        df.reset_index(drop=True, inplace=True)
        df = df.sem_filter("According to picture of PEGI {rating}, check if the game is only suitable for adults (18 years or older)", cascade_args=cascade_args)
        df.reset_index(drop=True, inplace=True)
        df = df.sem_filter("The {platforms} support both Windows and MacOS", cascade_args=cascade_args)
        df.reset_index(drop=True, inplace=True)
        df = df.sem_agg("Compute the average price in {original_price}.")
    ...
    if ROUND == 1:
        df.to_csv("workloads/steam_output/q10_out_lotus.csv", index=False)
        logger.info(f"saved output to workloads/steam_output/q10_out_lotus.csv...")
