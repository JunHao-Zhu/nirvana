...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/steam_lotus", log_name=f"q9_r{ROUND}")
    ...
    try:
        ...
        df = df.sem_filter("The {platforms} of the game support VR.", cascade_args=cascade_args)
        df.reset_index(drop=True, inplace=True)
        df = df.sem_filter("The {genre} of the game is about shooting.", cascade_args=cascade_args)
        df.reset_index(drop=True, inplace=True)
        df = df.sem_map("Extract the price in USD from the {discounted_price} of the game in IDR.", suffix="price_usd")
        df = df.sem_agg("Compute the average price in USD of {price_usd}.")
    ...
    if ROUND == 1:
        df.to_csv("workloads/steam_output/q9_out_lotus.csv", index=False)
        logger.info(f"saved output to workloads/steam_output/q9_out_lotus.csv...")
