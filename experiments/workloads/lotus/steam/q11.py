...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/steam_lotus", log_name=f"q11_r{ROUND}")
    ...
    logger.info(f"Q11: Find the earliest release date of a game supporting VR.")
    start_time = time.time()
    try:
        df = df.sem_filter("The {platforms} support VR.", cascade_args=cascade_args)
    ...
    if ROUND == 1:
        df.to_csv("workloads/steam_output/q11_out_lotus.csv", index=False)
        logger.info(f"saved output to workloads/steam_output/q11_out_lotus.csv...")
