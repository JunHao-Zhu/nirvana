...
if __name__ == "__main__":
    ...
    logger = create_logger(log_dir="log/imdb", log_name=f"q4_r{ROUND}{ablation_suffix}")
    ...
    df.semantic_filter(user_instruction="Whether the movie has ever won more than 3 Oscars?", input_column="Awards")
    ...
    if ROUND == 1:
        output.to_csv(f"workloads/imdb_output/q4_out{ablation_suffix}.csv", index=False)
        logger.info(f"saved output to workloads/imdb_output/q4_out{ablation_suffix}.csv...")
