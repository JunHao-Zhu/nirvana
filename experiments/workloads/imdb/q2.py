...
if __name__ == "__main__":
    ...
    logger = create_logger(log_dir="log/imdb", log_name=f"q2_r{ROUND}{ablation_suffix}")
    data = pd.read_csv("testdata/imdb_movie_info.csv")
    df = nvn.DataFrame(data)
    
    logger.info(f"Q2: Find all movies directed by Christopher Nolan.")
    df.semantic_filter(user_instruction="The movie is directed by Christopher Nolan.", input_column="Director")
    ...
    if ROUND == 1:
        output.to_csv(f"workloads/imdb_output/q2_out{ablation_suffix}.csv", index=False)
        logger.info(f"saved output to workloads/imdb_output/q2_out{ablation_suffix}.csv...")
