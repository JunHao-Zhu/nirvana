...
if __name__ == "__main__":
    ...
    logger = create_logger(log_dir="log/imdb", log_name=f"q3_r{ROUND}{ablation_suffix}")
    data = pd.read_csv("testdata/imdb_movie_info.csv")
    data["Poster"] = nvn.ImageArray(data["Poster"])
    df = nvn.DataFrame(data)

    logger.info(f"Q3: Find all movies whose poster is in the dark style.")
    df.semantic_filter(user_instruction="Whether the movie poster image is in the dark style.", input_column="Poster")
    ...
    if ROUND == 1:
        output.to_csv(f"workloads/imdb_output/q3_out{ablation_suffix}.csv", index=False)
        logger.info(f"saved output to workloads/imdb_output/q3_out{ablation_suffix}.csv...")
