...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/imdb_pz", log_name=f"q6_r{ROUND}")
    ...
    df = df.sem_filter(_filter="The movie is directed by Quentin Tarantino.", depends_on="Director")
    df = df.count()
    ...
    if ROUND == 1:
        output.to_df().to_csv("workloads/imdb_output/q6_out_pz.csv", index=False)
        logger.info(f"saved output to workloads/imdb_output/q6_out_pz.csv...")
