...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/imdb_pz", log_name=f"q2_r{ROUND}")
    data = pd.read_csv("testdata/imdb_movie_info.csv")
    df = pz.Dataset(data)

    logger.info(f"Q2: Find all movies directed by Christopher Nolan.")
    df = df.sem_filter("The movie is directed by Christopher Nolan.", depends_on="Director")
    ...
    if ROUND == 1:
        output.to_df().to_csv("workloads/imdb_output/q2_out_pz.csv", index=False)
        logger.info(f"saved output to workloads/imdb_output/q2_out_pz.csv...")
