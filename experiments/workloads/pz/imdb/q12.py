...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/imdb_pz", log_name=f"q12_r{ROUND}")
    ...
    new_column = [
        {"name": "Character", "type": str, "desc": "The main characters in the movie."}
    ]
    df = df.sem_add_columns(cols=new_column, depends_on="Plot", desc="Extract the main character from the movie plot.")
    ...
    if ROUND == 1:
        output.to_df().to_csv("workloads/imdb_output/q12_out_pz.csv", index=False)
        logger.info(f"saved output to workloads/imdb_output/q12_out_pz.csv...")
