...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/imdb_pz", log_name=f"q3_r{ROUND}")
    ...
    df = df.sem_filter("Whether the movie poster image is in the dark style.", depends_on="Poster")
    ...
    if ROUND == 1:
        output.to_df().to_csv("workloads/imdb_output/q3_out_pz.csv", index=False)
        logger.info(f"saved output to workloads/imdb_output/q3_out_pz.csv...")
