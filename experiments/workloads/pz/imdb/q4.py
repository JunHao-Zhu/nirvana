...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/imdb_pz", log_name=f"q4_r{ROUND}")
    ...
    df = df.sem_filter(_filter="Whether the movie has ever won more than 3 Oscars?", depends_on="Awards")
    ...
    if ROUND == 1:
        output.to_df().to_csv("workloads/imdb_output/q4_out_pz.csv", index=False)
        logger.info(f"saved output to workloads/imdb_output/q4_out_pz.csv...")
