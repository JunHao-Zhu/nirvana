...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/estate_pz", log_name=f"q3_r{ROUND}")
    ...
    df = df.sem_filter(_filter="Whether the house is located in Ajah, Lagos?", depends_on="Location")
    ...
    if ROUND == 1:
        output.to_df().to_csv("workloads/estate_output/q3_out_pz.csv", index=False)
        logger.info(f"saved output to workloads/estate_output/q3_out_pz.csv...")
