...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/estate_pz", log_name=f"q7_r{ROUND}")
    ...
    logger.info(f"Q7: Extract the features of the estate that have 2 or 3 bedrooms.")
    new_column = [
        {"name": "Features", "type": str, "desc": "The estate features."}
    ]
    df = df.sem_add_columns(cols=new_column, depends_on="Details", desc="Extract features from the detail about the estate.")
    ...
    if ROUND == 1:
        output.to_df().to_csv("workloads/estate_output/q7_out_pz.csv", index=False)
        logger.info(f"saved output to workloads/estate_output/q7_out_pz.csv...")
