...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/estate_pz", log_name=f"q10_r{ROUND}")
    ...
    logger.info(f"Q10: Compute the lowest price for detached duplex with more than 3 bedrooms and less than 6 bedrooms.")
    new_column = [
    ...
    df = df.sem_filter(_filter="Whether the estate has more than 3 bedrooms?", depends_on="Title")
    df = df.sem_filter(_filter="Whether the estate has less than 6 bedrooms?", depends_on="Title")
    df = df.sem_filter(_filter="Whether the estate is a detached house?", depends_on="Title")
    df = df.average()
    ...
    if ROUND == 1:
        output.to_df().to_csv("workloads/estate_output/q10_out_pz.csv", index=False)
        logger.info(f"saved output to workloads/estate_output/q10_out_pz.csv...")
