...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/steam_pz", log_name=f"q8_r{ROUND}")
    ...
    df = df.sem_filter("Does the video game has only one developer", depends_on="developer")
    ...
    if ROUND == 1:
        output.to_df().to_csv("workloads/steam_output/q8_out_pz.csv", index=False)
        logger.info(f"saved output to workloads/steam_output/q8_out_pz.csv...")
