...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/steam_pz", log_name=f"q3_r{ROUND}")
    data = pd.read_csv("testdata/steam_games.csv")
    df = pz.Dataset(data)
    df.schema.rating = ImageURLField(df.schema.rating.desc)

    logger.info(f"Q3: Does the video game support VR.")
    df = df.sem_filter("Does the video game support VR", depends_on="platforms")

    config = pz.QueryProcessorConfig(policy=pz.MinTime(), execution_strategy="parallel", progress=True)
    logger.info(f"Display the optimization config:\n{str(config)}")
    output = df.run(config)
    if ROUND == 1:
        output.to_df().to_csv("workloads/steam_output/q3_out_pz.csv", index=False)
        logger.info(f"saved output to workloads/steam_output/q3_out_pz.csv...")
