...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/steam_pz", log_name=f"q4_r{ROUND}")
    data = pd.read_csv("testdata/steam_games.csv")
    df = pz.Dataset(data)
    df.schema.image = ImageURLField(df.schema.image.desc)

    logger.info(f"Q4: Find all games with a Metacritic score higher than 90.")
    ...
    if ROUND == 1:
        output.to_df().to_csv("workloads/steam_output/q4_out_pz.csv", index=False)
        logger.info(f"saved output to workloads/steam_output/q4_out_pz.csv...")
