...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/steam_pz", log_name=f"q9_r{ROUND}")
    ...
    df = df.sem_filter("The video game supports VR.", depends_on="platforms")
    df = df.sem_filter("The video game is about shooting.", depends_on="genre")
    new_column = [
        {"name": "price_usd", "type": str, "desc": "The game's price in USD."}
    ]
    df = df.sem_add_columns(cols=new_column, depends_on="discounted_price", desc="Convert the price in IDR into the price in USD.")
    ...
    if ROUND == 1:
        output.to_df().to_csv("workloads/steam_output/q9_out_pz.csv", index=False)
        logger.info(f"saved output to workloads/steam_output/q9_out_pz.csv...")
