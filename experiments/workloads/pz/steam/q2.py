...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/steam_pz", log_name=f"q2_r{ROUND}")
    data = pd.read_csv("testdata/steam_games.csv")
    df = pz.Dataset(data)
    df.schema.rating = ImageURLField(df.schema.rating.desc)

    logger.info(f"Q2: Convert overall reviews into binary reviews.")
    new_column = [
    ...
    if ROUND == 1:
        output.to_df().to_csv("workloads/steam_output/q2_out_pz.csv", index=False)
        logger.info(f"saved output to workloads/steam_output/q2_out_pz.csv...")
