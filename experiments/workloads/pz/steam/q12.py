...
if __name__ == "__main__":
    logger = create_logger(log_dir="log/steam_pz", log_name=f"q12_r{ROUND}")
    data = pd.read_csv("testdata/steam_games.csv")
    df = pz.Dataset(data)
    df.schema.image = ImageURLField(df.schema.image.desc)

    logger.info(f"Q12: Find the earliest release date of an adventure game with a metacritics score higher than 90.")
    df = df.sem_filter("The rating is higher than 90", depends_on="metacriticts")
    new_column = [
    ...
    df = df.sem_filter("The video game is about adventure.", depends_on="genre")
    ...
    if ROUND == 1:
        output.to_df().to_csv("workloads/steam_output/q12_out_pz.csv", index=False)
        logger.info(f"saved output to workloads/steam_output/q12_out_pz.csv...")
