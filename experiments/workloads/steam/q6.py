...
if __name__ == "__main__":
    ...
    logger = create_logger(log_dir="log/steam", log_name=f"q6_r{ROUND}{ablation_suffix}")
    df = nv.DataFrame.from_external_file("testdata/steam_games.csv")
    
    logger.info(f"Q6: Compute the lowest discounted price of the video game that support MacOS.")
    df.semantic_filter(user_instruction="Is MacOS in the list of supported platforms?", input_columns=["platforms"])
    df.semantic_reduce(user_instruction="Find the lowest price.", input_column="discounted_price")
    ...
    if ROUND == 1:
        output.to_csv(f"workloads/steam_output/q6_out{ablation_suffix}.csv", index=False)
        logger.info(f"saved output to workloads/steam_output/q6_out{ablation_suffix}.csv...")
