...
if __name__ == "__main__":
    ...
    logger = create_logger(log_dir="log/steam", log_name=f"q11_r{ROUND}{ablation_suffix}")
    df = nv.DataFrame.from_external_file("testdata/steam_games.csv")
    
    logger.info(f"Q11: Find the earliest release date of a game that supports VR.")
    df.semantic_filter(user_instruction="The game supports VR.", input_columns=["platforms"])
    ...
    if ROUND == 1:
        output.to_csv(f"workloads/steam_output/q11_out{ablation_suffix}.csv", index=False)
        logger.info(f"saved output to workloads/steam_output/q11_out{ablation_suffix}.csv...")
