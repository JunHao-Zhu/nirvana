...
if __name__ == "__main__":
    ...
    logger = create_logger(log_dir="log/steam", log_name=f"q5_r{ROUND}{ablation_suffix}")
    ...
    df.semantic_filter(user_instruction="The video game is about sports.", input_columns=["genre"])
    df.semantic_reduce(user_instruction="Find the publisher that appears the most.", input_column="publisher")
    ...
    if ROUND == 1:
        output.to_csv(f"workloads/steam_output/q5_out{ablation_suffix}.csv", index=False)
        logger.info(f"saved output to workloads/steam_output/q5_out{ablation_suffix}.csv...")
