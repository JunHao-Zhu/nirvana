...
if __name__ == "__main__":
    ...
    logger = create_logger(log_dir="log/steam", log_name=f"q3_r{ROUND}{ablation_suffix}")
    df = nv.DataFrame.from_external_file("testdata/steam_games.csv")
    
    logger.info(f"Q3: Does the video game support VR.")
    df.semantic_filter(user_instruction="Does the video game support VR?", input_columns=["platforms"])
    ...
    if ROUND == 1:
        output.to_csv(f"workloads/steam_output/q3_out{ablation_suffix}.csv", index=False)
        logger.info(f"saved output to workloads/steam_output/q3_out{ablation_suffix}.csv...")
